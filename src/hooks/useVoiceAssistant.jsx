...
export const useVoiceAssistant = ({ geminiTools, onFunctionCall, onPlainText }) => {
  ...
  useEffect(() => {
    ...
    if (window.location.protocol !== 'https:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1') {
      const msg = "Microphone requires a secure connection (HTTPS). It will not work on HTTP/IP addresses.";
      console.error(msg);
      setError(msg);
      setIsDictationSupported(false);
      return;
    ...
    if (!SpeechRecognition) {
      setIsDictationSupported(false);
      setError("Voice dictation is not supported by this browser (Use Chrome or Edge).");
      return;
    }
      ...
      if (isListeningRef.current) {
        ...
        restartTimerRef.current = setTimeout(() => {
            try {
                ...
                // If restart fails (common in background tabs), reset UI
                console.warn("Restart failed:", e);
                setVoiceStatus('idle');
                isListeningRef.current = false; 
            }
    ...
    recognition.onerror = (event) => {
      ...
      if (event.error === 'not-allowed' || event.error === 'service-not-allowed') {
          setError(`Microphone access blocked. Check browser permissions or HTTPS settings.`);
          isListeningRef.current = false; 
          setVoiceStatus('idle');
  ...
  const handleToggleListening = useCallback(() => {
    ...
    if (isListeningRef.current) {
      ...
      try {
        setError(null);
        // This start must happen inside the click event for mobile support
        recognitionRef.current.start();
        isListeningRef.current = true;
        setVoiceStatus('listening'); 
      } catch (e) {
