...
//             if (currentUser) {
//                 // Fetch basic profile to have it available in the app (e.g. for physician name)
...
//                             setUserProfile(data);
//                             // Auto-fill referring physician if available and it's a new session default
//                             if (data.displayName && referringPhysician === 'Dr. Evelyn Reed') {
//                                 // Or we could have a separate field for "Reporting Radiologist"
//                             }
//                         }
//                     } catch (err) {
...
//                     ${userProfile ? `<tr><td style="padding: 8px; border: 1px solid #e2e8f0; font-weight: bold; background-color: #f8fafc;">Reporting Radiologist</td><td style="padding: 8px; border: 1px solid #e2e8f0;" colspan="3">${userProfile.displayName || ''} ${userProfile.title ? `(${userProfile.title})` : ''} - ${userProfile.institution || ''}</td></tr>` : ''}
//                 </table>
//             </div>`;
...
//                         <span className="text-sm text-gray-600 hidden sm:inline">{userProfile?.displayName || user.email}</span>
//                         <button onClick={() => setShowProfile(true)} className="bg-white text-blue-600 font-semibold px-3 py-2 rounded-lg border border-blue-200 hover:bg-blue-50 transition shadow-sm flex items-center space-x-2"><User size={16} /> <span className="hidden md:inline">Profile</span></button>
...
//             {showProfile && <UserProfile user={user} onClose={() => { setShowProfile(false); /* Refresh profile locally if needed */ }} />}
