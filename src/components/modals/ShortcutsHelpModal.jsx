...
export default function ShortcutsHelpModal({ shortcuts = {}, onClose }) {
  ...
  const alt = isMac ? "⌥" : "Alt";
  const Key = ({ k }) => (
    <kbd className="px-2 py-1.5 text-xs font-semibold text-foreground bg-muted border border-border rounded shadow-sm">
      {k}
    </kbd>
  );
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-0 md:p-4">
      <div className="bg-background rounded-none md:rounded-2xl shadow-2xl w-full h-full md:h-auto md:max-w-2xl md:max-h-[90vh] flex flex-col border border-border">
        ...
        <div className="p-6 overflow-y-auto grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
          ...
          {!Object.keys(shortcuts).length && (
            <div className="text-muted-foreground text-sm">No shortcuts configured.</div>
          )}
        </div>
      </div>
    </div>
