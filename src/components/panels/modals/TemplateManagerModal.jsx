...
import React, { useState, useEffect } from 'react';
import { XCircle, Plus, Trash2, FileText, Bold } from 'lucide-react';
import { db } from '../../firebase'; // Assuming firebase.js is set up
import { collection, addDoc, deleteDoc, doc, onSnapshot, serverTimestamp } from "firebase/firestore";
import { toast } from 'react-hot-toast';
...
const TemplateManagerModal = ({ user, existingModalities, onClose }) => {
  ...
  const editor = useEditor({
    ...
    content: '<p>Start writing your template here. Use the bold button for headings like <strong>LIVER:</strong> or <strong>IMPRESSION:</strong></p>',
    editorProps: {
      attributes: {
        class: 'prose prose-sm focus:outline-none w-full p-2 border rounded-b-lg min-h-[250px] text-black strong-2 text-black',
      },
    },
  });
  ...
  const handleAddTemplate = async () => {
    ...
    // Get content from the Tiptap editor
    ...
    try {
      await addDoc(collection(db, "users", user.uid, "templates"), {
        modality: newTemplateModality,
        name: newTemplateName,
        content: newTemplateContent, // Use the HTML from the editor
        createdAt: serverTimestamp(),
      });
      toast.success("Template added successfully!");
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="border rounded-lg">
                <div className="flex items-center p-2 bg-gray-100 rounded-t-lg border-b text-black">
                  <button
                  ...
                  {/* Add more buttons here for italics, lists, etc. */}
                </div>
